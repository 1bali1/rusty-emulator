...
type InstructionFn = fn(&mut CPU) -> u8;
...
impl CPU
{
    ...
    fn executeInterrupt(&mut self, bitIndex: u8)
    {
        self.isHalted = false;
    ...
    {
        let low = self.bus.read(self.registers.sp) as u16;
        self.registers.sp = self.registers.sp.wrapping_add(1);

        let high = self.bus.read(self.registers.sp) as u16;
        self.registers.sp = self.registers.sp.wrapping_add(1);
        
        return low | high << 8;
    }

    fn pushU16(&mut self, value: u16)
    {
        let high = (value >> 8) as u8;
        let low = (value & 0xff) as u8;

        self.registers.sp = self.registers.sp.wrapping_sub(1);
        self.bus.write(self.registers.sp, high);

        self.registers.sp = self.registers.sp.wrapping_sub(1);
        self.bus.write(self.registers.sp, low);
    }

    fn callFn(&mut self, targetAddress: u16)
    {
        let address = self.registers.pc;

        self.pushU16(address);

        self.registers.pc = targetAddress;
    }

    fn rst(&mut self, target: u16)
    {
        let address = self.registers.pc;
        self.pushU16(address);
    ...
    fn rlcAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
        let res = self.rlc(val);
        self.bus.write(address, res);
    }

    fn rrcAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
    ...
    fn rlAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
        let res = self.rl(val);
        self.bus.write(address, res);
    ...
    fn rrAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
        let res = self.rr(val);
        self.bus.write(address, res);
    ...
    fn slaAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
    ...
    fn sraAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
    ...
    fn srlAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
    ...
    fn bitAddress(&mut self, address: u16, index: u8)
    {
        let val = self.bus.read(address);
        self.bit(val, index);
    }

    fn resBitAddress(&mut self, address: u16, index: u8)
    {
        let val = self.bus.read(address);
    ...
    fn setBitAddress(&mut self, address: u16, index: u8)
    {
        let val = self.bus.read(address);
    ...
    fn swapAddress(&mut self, address: u16)
    {
        let val = self.bus.read(address);
    ...
    fn ldAddr16Sp(&mut self) -> u8
    {
        ...
        self.bus.write(address, low);
        self.bus.write(address.wrapping_add(1), high);

        return 20;
    }

    // ADD HL, BC | 1  8 | - 0 H C
    fn addHlBc(&mut self) -> u8
    {
    ...
    // LD [HL+], A | 1  8 | - - - -
    fn ldHlPlusAddressA(&mut self) -> u8
    {
    ...
    // LD A, [HL+] | 1  8 | - - - -
    fn ldAAddressHlPlus(&mut self) -> u8
    {
        let hl = self.registers.getHl();
        let val = self.bus.read(hl);
    ...
    // LD [HL-], A | 1  8 | - - - -
    fn ldHlMinusAddressA(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.a);
    ...
    // INC [HL] | 1  12 | Z 0 H -
    fn incAddressHl(&mut self) -> u8
    {
        let address = self.registers.getHl();
        let val = self.bus.read(address);
        let incdVal = self.incU8(val);

        self.bus.write(address, incdVal);

        return 12;
    }

    // DEC [HL] | 1  12 | Z 1 H -
    fn decAddressHl(&mut self) -> u8
    {
        let address = self.registers.getHl();
        let val = self.bus.read(address);
        let decdVal = self.decU8(val);

        self.bus.write(address, decdVal);

        return 12;
    }

    // LD [HL], n8 | 2  12 | - - - -
    fn ldAddressHl(&mut self) -> u8
    {
        let address = self.registers.getHl();
        let val = self.fetch();
    ...
    // LD A, [HL-] | 1  8 | - - - -
    fn ldAAddressHlMinus(&mut self) -> u8
    {
        let address = self.registers.getHl();
        let val = self.bus.read(address);
    ...
    // LD [HL], B | 1  8 | - - - -
    fn ldAddressHlB(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.b);

        return 8;
    }

    // LD [HL], C | 1  8 | - - - -
    fn ldAddressHlC(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.c);

        return 8;
    }

    // LD [HL], D | 1  8 | - - - -
    fn ldAddressHlD(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.d);

        return 8;
    }

    // LD [HL], E | 1  8 | - - - -
    fn ldAddressHlE(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.e);
        
        return 8;
    }

    // LD [HL], H | 1  8 | - - - -
    fn ldAddressHlH(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.h);

        return 8;
    }

    // LD [HL], L | 1  8 | - - - -
    fn ldAddressHlL(&mut self) -> u8
    {
        let address = self.registers.getHl();
        self.bus.write(address, self.registers.l);
    ...
    fn addAAddressHl(&mut self) -> u8
    {
        let address = self.registers.getHl();
        let x = self.bus.read(address);
    ...
    fn retNz(&mut self) -> u8
    {
        ...
        self.registers.pc = self.popU16();

        return 20;
    }

    // POP BC | 1  12 | - - - -
    fn popBc(&mut self) -> u8
    {
        let val = self.popU16();
        self.registers.setBc(val);
    ...
    // CALL NZ, a16 | 3  24/12 | - - - -
    fn callNz(&mut self) -> u8
    {
        let flag = self.registers.getFlag(Registers::MASK_ZERO_Z);

        let targetAddress = self.fetchU16();
        ...
        self.callFn(targetAddress);

        return 24;
    }

    // PUSH BC | 1  16 | - - - -
    fn pushBc(&mut self) -> u8
    {
        self.pushU16(self.registers.getBc());
    ...
    // PUSH DE | 1  16 | - - - -
    fn pushDe(&mut self) -> u8
    {
        self.pushU16(self.registers.getDe());
    ...
    // PUSH HL | 1  16 | - - - -
    fn pushHl(&mut self) -> u8
    {
        self.pushU16(self.registers.getHl());
    ...
    // PUSH AF | 1  16 | - - - -
    fn pushAf(&mut self) -> u8
    {
        self.pushU16(self.registers.getAf());
    ...
    fn ldHlSpPlusByte(&mut self) -> u8
    {
        let x = self.fetch() as i8;
        let sp = self.registers.sp;
    ...
    // RLC [HL] | 2  16 | Z 0 0 C
    fn rlcAddressHl(&mut self) -> u8
    {
        self.rlcAddress(self.registers.getHl());
    ...
    // RRC [HL] | 2  8 | Z 0 0 C
    fn rrcAddressHl(&mut self) -> u8
    {
        self.rrcAddress(self.registers.getHl());
    ...
    // RL [HL] | 2  16 | Z 0 0 C
    fn rlAddressHl(&mut self) -> u8
    {
        self.rlAddress(self.registers.getHl());
    ...
    // RR [HL] | 2  16 | Z 0 0 C
    fn rrAddressHl(&mut self) -> u8
    {
        self.rrAddress(self.registers.getHl());
    ...
    // SLA [HL] | 2  16 | Z 0 0 C
    fn slaAddressHl(&mut self) -> u8
    {
        self.slaAddress(self.registers.getHl());
    ...
    // SRA [HL] | 2  16 | Z 0 0 C
    fn sraAddressHl(&mut self) -> u8
    {
        self.sraAddress(self.registers.getHl());
    ...
    fn swapAddressHl(&mut self) -> u8
    {
        self.swapAddress(self.registers.getHl());
        
        return 16;
    }

    // SWAP A | 2  8 | Z 0 0 0
    fn swapA(&mut self) -> u8
    {
    ...
    // SRL [HL] | 2  16 | Z 0 0 C
    fn srlAddressHl(&mut self) -> u8
    {
        self.srlAddress(self.registers.getHl());
    ...
    // BIT 0, [HL] | 2  12 | Z 0 1 -
    fn bit0AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 0);
        
        return 12;
    }

    // BIT 0, A | 2  8 | Z 0 1 -
    fn bit0A(&mut self) -> u8
    {
    ...
    // BIT 1, [HL] | 2  12 | Z 0 1 -
    fn bit1AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 1);
        
        return 12;
    }

    // BIT 1, A | 2  8 | Z 0 1 -
    fn bit1A(&mut self) -> u8
    {
    ...
    // BIT 2, [HL] | 2  12 | Z 0 1 -
    fn bit2AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 2);
        
        return 12;
    }

    // BIT 2, A | 2  8 | Z 0 1 -
    fn bit2A(&mut self) -> u8
    {
    ...
    // BIT 3, [HL] | 2  12 | Z 0 1 -
    fn bit3AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 3);
        
        return 12;
    }

    // BIT 3, A | 2  8 | Z 0 1 -
    fn bit3A(&mut self) -> u8
    {
    ...
    // BIT 4, [HL] | 2  12 | Z 0 1 -
    fn bit4AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 4);
        
        return 12;
    }

    // BIT 4, A | 2  8 | Z 0 1 -
    fn bit4A(&mut self) -> u8
    {
    ...
    // BIT 5, [HL] | 2  12 | Z 0 1 -
    fn bit5AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 5);
        
        return 12;
    }

    // BIT 5, A | 2  8 | Z 0 1 -
    fn bit5A(&mut self) -> u8
    {
    ...
    // BIT 6, [HL] | 2  12 | Z 0 1 -
    fn bit6AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 6);
        
        return 12;
    }

    // BIT 6, A | 2  8 | Z 0 1 -
    fn bit6A(&mut self) -> u8
    {
    ...
    // BIT 7, [HL] | 2  12 | Z 0 1 -
    fn bit7AddressHl(&mut self) -> u8
    {
        self.bitAddress(self.registers.getHl(), 7);
        
        return 12;
    }

    // BIT 7, A | 2  8 | Z 0 1 -
    fn bit7A(&mut self) -> u8
    {
    ...
    // RES 0, [HL] | 2  16 | - - - -
    fn res0AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 0);
        
        return 16;
    }

    // RES 0, A | 2  8 | - - - -
    fn res0A(&mut self) -> u8
    {
    ...
    // RES 1, [HL] | 2  16 | - - - -
    fn res1AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 1);
        
        return 16;
    }

    // RES 1, A | 2  8 | - - - -
    fn res1A(&mut self) -> u8
    {
    ...
    // RES 2, [HL] | 2  16 | - - - -
    fn res2AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 2);
        
        return 16;
    }

    // RES 2, A | 2  8 | - - - -
    fn res2A(&mut self) -> u8
    {
    ...
    // RES 3, [HL] | 2  16 | - - - -
    fn res3AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 3);
        
        return 16;
    }

    // RES 3, A | 2  8 | - - - -
    fn res3A(&mut self) -> u8
    {
    ...
    // RES 4, [HL] | 2  16 | - - - -
    fn res4AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 4);
        
        return 16;
    }

    // RES 4, A | 2  8 | - - - -
    fn res4A(&mut self) -> u8
    {
    ...
    // RES 5, [HL] | 2  16 | - - - -
    fn res5AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 5);
        
        return 16;
    }

    // RES 5, A | 2  8 | - - - -
    fn res5A(&mut self) -> u8
    {
    ...
    // RES 6, [HL] | 2  16 | - - - -
    fn res6AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 6);
        
        return 16;
    }

    // RES 6, A | 2  8 | - - - -
    fn res6A(&mut self) -> u8
    {
    ...
    // RES 7, [HL] | 2  16 | - - - -
    fn res7AddressHl(&mut self) -> u8
    {
        self.resBitAddress(self.registers.getHl(), 7);
        
        return 16;
    }

    // RES 7, A | 2  8 | - - - -
    fn res7A(&mut self) -> u8
    {
    ...
    // SET 0, [HL] | 2  16 | - - - -
    fn set0AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 0);
        
        return 16;
    }

    // SET 0, A | 2  8 | - - - -
    fn set0A(&mut self) -> u8
    {
    ...
    // SET 1, [HL] | 2  16 | - - - -
    fn set1AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 1);
        
        return 16;
    }

    // SET 1, A | 2  8 | - - - -
    fn set1A(&mut self) -> u8
    {
    ...
    // SET 2, [HL] | 2  16 | - - - -
    fn set2AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 2);
        
        return 16;
    }

    // SET 2, A | 2  8 | - - - -
    fn set2A(&mut self) -> u8
    {
    ...
    // SET 3, [HL] | 2  16 | - - - -
    fn set3AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 3);
        
        return 16;
    }

    // SET 3, A | 2  8 | - - - -
    fn set3A(&mut self) -> u8
    {
    ...
    // SET 4, [HL] | 2  16 | - - - -
    fn set4AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 4);
        
        return 16;
    }

    // SET 4, A | 2  8 | - - - -
    fn set4A(&mut self) -> u8
    {
    ...
    // SET 5, [HL] | 2  16 | - - - -
    fn set5AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 5);
        
        return 16;
    }

    // SET 5, A | 2  8 | - - - -
    fn set5A(&mut self) -> u8
    {
    ...
    // SET 6, [HL] | 2  16 | - - - -
    fn set6AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 6);
        
        return 16;
    }

    // SET 6, A | 2  8 | - - - -
    fn set6A(&mut self) -> u8
    {
    ...
    // SET 7, [HL] | 2  16 | - - - -
    fn set7AddressHl(&mut self) -> u8
    {
        self.setBitAddress(self.registers.getHl(), 7);
        
        return 16;
    }

    // SET 7, A | 2  8 | - - - -
    fn set7A(&mut self) -> u8
    {
    ...
    {
        let opcode = self.fetch();
        let clockCycle = self.instructions[opcode as usize](self);
